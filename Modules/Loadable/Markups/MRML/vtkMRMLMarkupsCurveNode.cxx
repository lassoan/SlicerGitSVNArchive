  ...
  double samplingDistance, bool closedCurve)
{
  ...
  // or instability in other algorithms due to points being too close to each other.
  // To fix that, move the last point to have two equal segments at the end.
  bool makeLastTwoSegmentsEqualLength = false;
  if (closedCurve)
    {
    if (sampledPoints->GetNumberOfPoints() > 3)
      {
      makeLastTwoSegmentsEqualLength = true;
      }
    }
  else
    {
    double originalLastPoint[3] = { 0.0 };
    originalPoints->GetPoint(sampledPoints->GetNumberOfPoints() - 1, originalLastPoint);
  ...
  if (makeLastTwoSegmentsEqualLength)
    {
    double firstPoint[3] = { 0.0 };
    double secondLastPoint[3] = { 0.0 };
    double lastPoint[3] = { 0.0 };
