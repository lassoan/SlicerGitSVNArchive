  ...
  virtual void Execute(vtkObject *vtkNotUsed(caller), unsigned long event, void *callData)
  {
    ...
    if (this->DisplayableManager->Is2DDisplayableManager())
    {
      ...
      if (event == vtkCommand::InteractionEvent)
      {
        ...
        vtkMarkupsRepresentation * representation = vtkMarkupsRepresentation::SafeDownCast(this->Widget->GetRepresentation());
        if (!representation)
        {
...
//---------------------------------------------------------------------------
vtkMRMLMarkupsFiducialDisplayableManager::vtkMRMLMarkupsFiducialDisplayableManager()
{
...
vtkMarkupsWidget * vtkMRMLMarkupsFiducialDisplayableManager::CreateWidget(vtkMRMLMarkupsNode* markupsNode)
{
  ...
  vtkSmartPointer<vtkMarkupsRepresentation> rep;
  ...
  {
    rep = vtkSmartPointer<vtkMarkupsSplineRepresentation>::New();
  }
  else
  {
    rep = vtkSmartPointer<vtkMarkupsRepresentation>::New();
  }
...
bool vtkMRMLMarkupsFiducialDisplayableManager::UpdateNthHandlePositionFromMRML(int n, vtkMarkupsWidget *markupsWidget, vtkMRMLMarkupsNode *pointsNode)
{
  ...
  if (this->Is2DDisplayableManager())
  {
    ...
    markupsRepresentation->GetHandlePosition(n, displayCoordinatesBuffer1);
    ...
    if (this->GetDisplayCoordinatesChanged(displayCoordinates1, displayCoordinatesBuffer1))
    {
      ...
      {
        markupsRepresentation->SetHandlePosition(n, displayCoordinates1);
  ...
  else
  {
    ...
    markupsRepresentation->GetHandlePosition(n, handleWorldCoord);

    if (this->GetWorldCoordinatesChanged(handleWorldCoord, fidWorldCoord))
    {
      markupsRepresentation->SetHandlePosition(n, fidWorldCoord);
      positionChanged = true;
    }
    else
    {
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsFiducialDisplayableManager::SetNthHandle(int n, vtkMRMLMarkupsNode* markupsNode, vtkMarkupsWidget *markupsWidget)
{
  vtkMarkupsRepresentation * markupsRepresentation = vtkMarkupsRepresentation::SafeDownCast(markupsWidget->GetRepresentation());
  if (!markupsRepresentation)
  {
  ...
  if (n >= numberOfHandles)
  {
    ...
    if (this->Is2DDisplayableManager())
    {
      ...
      newHandleIndex = markupsWidget->AddHandle(displayCoordinates);
    }
    else
    {
      double fidWorldCoord[4] = { 0., 0., 0., 1. };
      markupsNode->GetMarkupPointWorld(n, 0, fidWorldCoord);
      newHandleIndex = markupsWidget->AddHandle(fidWorldCoord);
    }
  ...
  if (this->Is2DDisplayableManager())
  {
    if (displayNode->GetVisibility() == 0 ||
      markupsNode->GetNthMarkupVisibility(n) == 0 ||
  ...
  // update the text
  std::string textString = markupsNode->GetNthMarkupLabel(n);
  ...
  if (prop)
  {
    double* color = markupsNode->GetNthMarkupSelected(n) ?
  ...
  if (fidVisible)
  {
    ...
    vtkHandleWidget* newHandle = markupsWidget->GetHandle(n);
    if (newHandle)
    {
    ...
    {
      vtkErrorMacro("Failed to get handle");
    }
  ...
  else
  {
    ...
    if (this->Is2DDisplayableManager())
    {
        ...
        markupsNode->GetDisplayNode())
      {
        ...
        markupsNode->GetMarkupPointWorld(n, 0, transformedP1);
          ...
          pointDisplayNode->GetVisibility())
        {
          ...
          if (!projectionSeed)
          {
            ...
            projectionSeed->AddHandle(displayP1);
...
void vtkMRMLMarkupsFiducialDisplayableManager::PropagateMRMLToWidget(vtkMRMLMarkupsNode* markupsNode, vtkMarkupsWidget * markupsWidget)
{
  if (!markupsWidget)
  {
...
void vtkMRMLMarkupsFiducialDisplayableManager::PropagateWidgetToMRML(vtkMarkupsWidget * markupsWidget, vtkMRMLMarkupsNode* markupsNode)
{
  ...
  if (markupsWidget->GetWidgetState() == vtkMarkupsWidget::Idle)
  {
    return;
  }
  ...
  {
    markupsNode->RemoveMarkup(markupsNode->GetNumberOfMarkups() - 1);
  ...
  for (int n = 0; n < numberOfHandles; n++)
  {
    ...
    if (this->Is2DDisplayableManager())
    {
      ...
      markupsRepresentation->GetHandlePosition(n, displayCoordinates);
      this->GetDisplayToWorldCoordinates(displayCoordinates, newCoords);
    }
    else
    {
      markupsRepresentation->GetHandlePosition(n, newCoords);
    }
...
void vtkMRMLMarkupsFiducialDisplayableManager::OnClickInRenderWindow(double x, double y, const char *associatedNodeID)
{
  ...
  double displayCoordinates1[2] = { x, y };
  ...
  vtkSmartPointer<vtkMRMLMarkupsNode> activeMarkupsNode;

  const char* activePlaceNodeClassName = this->GetActivePlaceNodeClassName();
  if (activePlaceNodeClassName == NULL)
  {
    vtkWarningMacro("OnClickInRenderWindow: invalid place node class name");
    return;
  }
  ...
  if (selectionNode)
  {
    ...
    if (mrmlNode && strcmp(mrmlNode->GetClassName(), activePlaceNodeClassName) == 0)
    {
  ...
  if (!activeMarkupsNode)
  {
    ...
    activeMarkupsNode = vtkSmartPointer<vtkMRMLMarkupsNode>::Take(
      vtkMRMLMarkupsNode::SafeDownCast(
      this->GetMRMLScene()->CreateNodeByClass(activePlaceNodeClassName)));
    if (activeMarkupsNode->IsA("vtkMRMLMarkupsFiducialNode"))
    {
      activeMarkupsNode->SetName(this->GetMRMLScene()->GetUniqueNameByString("F"));
    }
  }
  ...
  if (associatedNodeID)
  {
    vtkMRMLMarkupsFiducialNode* activeMarkupsFiducialNode = vtkMRMLMarkupsFiducialNode::SafeDownCast(activeMarkupsNode);
    if (activeMarkupsFiducialNode)
    {
      // TODO: maybe add this to the markup base class
      activeMarkupsFiducialNode->SetNthFiducialAssociatedNodeID(fiducialIndex, associatedNodeID);
    }
  }
...
void vtkMRMLMarkupsFiducialDisplayableManager::OnMRMLMarkupsNodeNthMarkupModifiedEvent(vtkMRMLMarkupsNode* node, int n)
{
  ...
  this->SetNthHandle(n, node, markupsWidget);
}
