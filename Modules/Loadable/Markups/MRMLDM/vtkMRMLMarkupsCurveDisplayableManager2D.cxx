...
#include "vtkMRMLMarkupsCurveDisplayableManager2D.h"
...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkMRMLMarkupsCurveDisplayableManager2D);

//---------------------------------------------------------------------------
// vtkMRMLMarkupsCurveDisplayableManager2D Callback
...
//---------------------------------------------------------------------------
// vtkMRMLMarkupsCurveDisplayableManager2D methods

//---------------------------------------------------------------------------
void vtkMRMLMarkupsCurveDisplayableManager2D::PrintSelf(ostream& os, vtkIndent indent)
{
...
vtkAbstractWidget * vtkMRMLMarkupsCurveDisplayableManager2D::CreateWidget(vtkMRMLMarkupsNode* node)
{
  ...
  if (!curveNode)
    {
    vtkErrorMacro("CreateWidget: Could not get curve node!")
    return 0;
    }
  ...
  if (!this->IsInLightboxMode())
    ...
    {
    int lightboxIndex = this->GetLightboxIndex(curveNode, 0,0);
...
void vtkMRMLMarkupsCurveDisplayableManager2D::OnWidgetCreated(vtkAbstractWidget * widget, vtkMRMLMarkupsNode * node)
{
...
void vtkMRMLMarkupsCurveDisplayableManager2D::SetNthSeed(int n, vtkMRMLMarkupsCurveNode* curveNode, vtkMarkupsCurveWidget *seedWidget)
{
  ...
  //    vtkSeedWidget* fiducialSeed = vtkSeedWidget::SafeDownCast(this->Helper->GetPointProjectionWidget(curveNode->GetNthMarkupID(n)));
  ...
  //      curveNode->GetNthFiducialWorldCoordinates(n, transformedP1);

  //      double displayP1[4];
  //      this->GetWorldToDisplayCoordinates(transformedP1, displayP1);

  //      vtkSeedWidget* projectionSeed =
  //        vtkSeedWidget::SafeDownCast(this->Helper->GetPointProjectionWidget(curveNode->GetNthMarkupID(n)));

  //      vtkMRMLMarkupsDisplayNode* pointDisplayNode = curveNode->GetMarkupsDisplayNode();
  ...
  //          {
  //          if (curveNode->GetNthMarkupSelected(n))
  //            {
...
void vtkMRMLMarkupsCurveDisplayableManager2D::OnClickInRenderWindow(double x, double y, const char *associatedNodeID)
{
...
void vtkMRMLMarkupsCurveDisplayableManager2D::AdditionnalInitializeStep()
{
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsCurveDisplayableManager2D::OnInteractorStyleEvent(int eventid)
{
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsCurveDisplayableManager2D::OnMRMLSceneEndClose()
{
