...
//---------------------------------------------------------------------------
/// Create a new widget.
vtkAbstractWidget * vtkMRMLMarkupsAngleDisplayableManager3D::CreateWidget(vtkMRMLMarkupsNode* node)
{
  if (!node)
    {
  ...
  vtkMRMLMarkupsAngleNode* fiducialNode = vtkMRMLMarkupsAngleNode::SafeDownCast(node);
  ...
  //seed widget
  vtkAngleWidget * seedWidget = vtkAngleWidget::New();
  ...
  seedWidget->SetWidgetStateToManipulate();
...
void vtkMRMLMarkupsAngleDisplayableManager3D::OnWidgetCreated(vtkAbstractWidget * widget, vtkMRMLMarkupsNode * node)
{
  if (!widget)
    {
...
//---------------------------------------------------------------------------
bool vtkMRMLMarkupsAngleDisplayableManager3D::UpdateNthSeedPositionFromMRML(int n, vtkAbstractWidget *widget, vtkMRMLMarkupsNode *pointsNode)
{
  ...
  vtkAngleWidget *seedWidget = vtkAngleWidget::SafeDownCast(widget);
  if (!seedWidget)
    {
    return false;
    }
  vtkAngleRepresentation3D * seedRepresentation = vtkAngleRepresentation3D::SafeDownCast(seedWidget->GetRepresentation());
  ...
  this->GetSeedWorldPosition(seedRepresentation, n, seedWorldCoord);
  ...
  if (this->GetWorldCoordinatesChanged(seedWorldCoord, fidWorldCoord))
    ...
    vtkDebugMacro("UpdateNthSeedPositionFromMRML: 3D:"
                  ...
                  << fidWorldCoord[1] << ", "
                  << fidWorldCoord[2]);
    this->SetSeedWorldPosition(seedRepresentation, n, fidWorldCoord);
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsAngleDisplayableManager3D::SetNthSeed(int n, vtkMRMLMarkupsAngleNode* fiducialNode, vtkAngleWidget *seedWidget)
{
  vtkAngleRepresentation3D * seedRepresentation = vtkAngleRepresentation3D::SafeDownCast(seedWidget->GetRepresentation());
  ...
  if (n >= 3)
  {
    vtkDebugMacro("SetNthSeed: n must be < 3");
    return;
  }
  vtkHandleRepresentation* handleRep = NULL;
  if (n == 0)
  {
    handleRep = seedRepresentation->GetPoint1Representation();
  }
  else if (n == 1)
  {
    handleRep = seedRepresentation->GetCenterRepresentation();
  }
  else if (n == 2)
  {
    handleRep = seedRepresentation->GetPoint2Representation();
  }
  ...
  if (!handleRep)
    {
    vtkErrorMacro("Failed to get an oriented polygonal handle rep for n = " << n);
    return;
    }

  // update the text
  std::string textString = fiducialNode->GetNthMarkupLabel(n);
  /*
  ...
  if (oldGlyphType != displayNode->GetGlyphType())
    {
...
void vtkMRMLMarkupsAngleDisplayableManager3D::PropagateMRMLToWidget(vtkMRMLMarkupsNode* node, vtkAbstractWidget * widget)
{
  if (!widget)
    {
  ...
  int numberOfAngles = fiducialNode->GetNumberOfMarkups();

  vtkDebugMacro("Fids PropagateMRMLToWidget, node num markups = " << numberOfAngles);
...
void vtkMRMLMarkupsAngleDisplayableManager3D::PropagateWidgetToMRML(vtkAbstractWidget * widget, vtkMRMLMarkupsNode* node)
{
  if (!widget)
    {
  ...
  if (seedWidget->GetWidgetState() != vtkCommand::InteractionEvent)
    {
  ...
  for (int n = 0; n < 3; n++)
    ...
    this->GetSeedWorldPosition(seedRepresentation, n, worldCoordinates1);
...
void vtkMRMLMarkupsAngleDisplayableManager3D::OnClickInRenderWindow(double x, double y, const char *associatedNodeID)
{
  ...
  // std::cout << "OnClickInRenderWindow: Setting " << fiducialIndex << "th fiducial label from " << activeAngleNode->GetNthAngleLabel(fiducialIndex);
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsAngleDisplayableManager3D::UpdatePosition(vtkAbstractWidget *widget, vtkMRMLNode *node)
{
//  vtkWarningMacro("UpdatePosition, node is " << (node == NULL ? "null" : node->GetID()));
  ...
  // now get the widget properties (coordinates, measurement etc.) and if the mrml node has changed, propagate the changes
  bool positionChanged = false;
  int numberOfAngles = pointsNode->GetNumberOfMarkups();
  for (int n = 0; n < numberOfAngles; n++)
    {
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsAngleDisplayableManager3D::OnMRMLMarkupsNodeNthMarkupModifiedEvent(vtkMRMLMarkupsNode* node, int n)
{
  ...
  this->SetNthSeed(n, vtkMRMLMarkupsAngleNode::SafeDownCast(node), seedWidget);
}

//---------------------------------------------------------------------------
void vtkMRMLMarkupsAngleDisplayableManager3D::OnMRMLMarkupsNodeMarkupAddedEvent(vtkMRMLMarkupsNode * markupsNode, int n)
{
  ...
  this->SetNthSeed(n, vtkMRMLMarkupsAngleNode::SafeDownCast(markupsNode), seedWidget);
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsAngleDisplayableManager3D::OnMRMLMarkupsNodeMarkupRemovedEvent(vtkMRMLMarkupsNode * markupsNode, int vtkNotUsed(n))
{
...
//---------------------------------------------------------------------------
void vtkMRMLMarkupsAngleDisplayableManager3D::GetSeedWorldPosition(vtkAngleRepresentation3D * seedRepresentation, int n, double* position)
{
  if (n == 0)
  {
    seedRepresentation->GetPoint1WorldPosition(position);
  }
  else if (n == 1)
  {
    seedRepresentation->GetCenterWorldPosition(position);
  }
  else if (n == 2)
  {
    seedRepresentation->GetPoint2WorldPosition(position);
  }
}

//---------------------------------------------------------------------------
void vtkMRMLMarkupsAngleDisplayableManager3D::SetSeedWorldPosition(vtkAngleRepresentation3D * seedRepresentation, int n, double* position)
{
  if (n == 0)
  {
    seedRepresentation->SetPoint1WorldPosition(position);
  }
  else if (n == 1)
  {
    seedRepresentation->SetCenterWorldPosition(position);
  }
  else if (n == 2)
  {
    seedRepresentation->SetPoint2WorldPosition(position);
  }
}
