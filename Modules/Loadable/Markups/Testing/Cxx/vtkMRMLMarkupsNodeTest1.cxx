...
int vtkMRMLMarkupsNodeTest1(int , char * [] )
{
  ...
  for (int p = -1; p < 3; p++)
    {
    TESTING_OUTPUT_ASSERT_ERRORS_BEGIN();
    if (node1->ControlPointExists(p))
      {
      std::cerr << "Says that point " << p << " exists, it shouldn't" << std::endl;
      return EXIT_FAILURE;
      }
    else
      {
      std::cout << "Points exists in markup: success on point " << p << std::endl;
      }
    TESTING_OUTPUT_ASSERT_ERRORS_END();
    }
  ...
  for (int n = -1; n < 3; n++)
    ...
    if (markup)
      {
      std::cout << "pass get nth markup " << n << std::endl;
      }
  ...
  std::string id = node1->GetNthControlPointAssociatedNodeID(2);
  ...
  bool retval = node1->ResetNthControlPointID(0);
  if (!retval)
    {
  ...
  // Selected/Visib
  //
  bool sel = node1->GetNthControlPointSelected();
  if (!sel)
    {
  ...
  if (node1->GetNthControlPointSelected(1))
    {
    std::cerr << "Error: markup 1 should be unselected" << std::endl;
    return EXIT_FAILURE;
    }
  bool visib = node1->GetNthControlPointVisibility();
  if (!visib)
    {
  ...
  if (node1->GetNthControlPointVisibility(1))
    {
  ...
  std::string labelTest = node1->GetNthControlPointLabel(1);
  ...
  if (commandLine1.size() != numMarkups)
    {
    std::cerr << "Incorrect number of command line args, for "
              << numMarkups << " markup points, got "
  ...
  if (commandLine2.size() != numMarkups)
    {
    std::cerr << "Incorrect number of command line args, for "
              << numMarkups << " markup points, got "
  ...
  unsigned int multipleFalseExpectedSize = 2*numMarkups;
