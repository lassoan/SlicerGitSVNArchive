...
class ScriptedLoadableModuleTest(unittest.TestCase):
  ...
  def delayDisplay(self,message,msec=1000):
    ...
    to the user) can be logged using slicer.logger.info function
    (printed to console and application log) or slicer.logger.debug
    function (printed to application log only).
    Error messages should be logged by slicer.logger.error function
  ...
  def runTest(self):
    ...
    """
    slicer.logger.warning('No test is defined in '+self.__class__.__name__)
