  ...
  vtkMRMLSegmentationDisplayNode* displayNode/*=NULL*/)
{
  ...
  if (maskSegmentIDs.empty())
    {
    // If we pass empty segment list to GenerateMergedLabelmap then it uses all segment IDs, so the result would not be a solid volume.
    // Therefore, we need to handle this special case separately here.
    maskImage->AllocateScalars(VTK_UNSIGNED_CHAR, 1);
    // it seems that actually 0 means writeable
  ...
  if (masterVolume != NULL && editableIntensityRange != NULL)
    ...
    // Where thresholdMask > 0, maskImage will be set to 0 (non-editable)
