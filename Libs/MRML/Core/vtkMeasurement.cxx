...
vtkStandardNewMacro(vtkMeasurement);

//----------------------------------------------------------------------------
vtkMeasurement::vtkMeasurement()
: Name(nullptr)
, Value(0.0)
, Units(nullptr)
, PrintFormat(nullptr)
...
//----------------------------------------------------------------------------
std::string vtkMeasurement::GetValueWithUnitsAsPrintableString()
{
  if (!this->PrintFormat)
    {
    return  "";
...
//----------------------------------------------------------------------------
void vtkMeasurement::Initialize()
{
...
//----------------------------------------------------------------------------
void vtkMeasurement::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkMeasurement::Copy(vtkMeasurement* src)
{
  if (!src)
    {
    return;
...
//----------------------------------------------------------------------------
std::string vtkMeasurement::GetAsString()
{
  std::string str;
  /* TODO: implement
  if (this->CodeValue)
...
//----------------------------------------------------------------------------
bool vtkMeasurement::SetFromString(const std::string& content)
{
  this->Initialize();
  bool success = true;
  /* TODO: implement
...
//----------------------------------------------------------------------------
void vtkMeasurement::SetQuantityCode(vtkCodedEntry* entry)
{
  if (!entry)
    {
    if (this->QuantityCode)
...
//----------------------------------------------------------------------------
void vtkMeasurement::SetDerivationCode(vtkCodedEntry* entry)
{
  if (!entry)
    {
    if (this->DerivationCode)
...
//----------------------------------------------------------------------------
void vtkMeasurement::SetUnitsCode(vtkCodedEntry* entry)
{
  if (!entry)
    {
    if (this->UnitsCode)
...
//----------------------------------------------------------------------------
void vtkMeasurement::SetMethodCode(vtkCodedEntry* entry)
{
  if (!entry)
    {
    if (this->MethodCode)
