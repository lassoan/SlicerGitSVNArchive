/*=========================================================================
  ...
  Program:   ParaView
  Module:    vtkDelimitedTextWriter2.cxx
...
#include "vtkDelimitedTextWriter2.h"
...
vtkStandardNewMacro(vtkDelimitedTextWriter2);
//-----------------------------------------------------------------------------
vtkDelimitedTextWriter2::vtkDelimitedTextWriter2()
{
...
//-----------------------------------------------------------------------------
int vtkDelimitedTextWriter2::FillInputPortInformation(
...
//-----------------------------------------------------------------------------
bool vtkDelimitedTextWriter2::OpenStream()
{
...
void vtkDelimitedTextWriter2GetDataString(
  iterT* iter, vtkIdType tupleIndex, ostream* stream, vtkDelimitedTextWriter2* writer,
  bool* first)
{
...
void vtkDelimitedTextWriter2GetDataString(
  ...
  ostream* stream, vtkDelimitedTextWriter2* writer, bool* first)
{
  int numComps = iter->GetNumberOfComponents();
  vtkIdType index = tupleIndex* numComps;
  for (int cc=0; cc < numComps; cc++)
...
//-----------------------------------------------------------------------------
vtkStdString vtkDelimitedTextWriter2::GetString(vtkStdString string)
{
...
//-----------------------------------------------------------------------------
void vtkDelimitedTextWriter2::WriteData()
{
...
//-----------------------------------------------------------------------------
void vtkDelimitedTextWriter2::WriteTable(vtkTable* table)
{
    ...
    for (iter = columnsIters.begin(); iter != columnsIters.end(); ++iter)
      ...
      switch ((*iter)->GetDataType())
        ...
        vtkArrayIteratorTemplateMacro(
          vtkDelimitedTextWriter2GetDataString(static_cast<VTK_TT*>(iter->GetPointer()),
            index, this->Stream, this, &first));
        case VTK_VARIANT:
          {
          vtkDelimitedTextWriter2GetDataString(static_cast<vtkArrayIteratorTemplate<vtkVariant>*>(iter->GetPointer()),
            index, this->Stream, this, &first);
          break;
          }
        }
...
//-----------------------------------------------------------------------------
char *vtkDelimitedTextWriter2::RegisterAndGetOutputString()
{
...
//-----------------------------------------------------------------------------
void vtkDelimitedTextWriter2::PrintSelf(ostream& os, vtkIndent indent)
{
